...
for fpath in lst_fpath:
    ...
    # Verify it's a 2D image
    if img.ndim != 2:
        raise ValueError(f"Expected 2D image, got {img.ndim}D data")
    ...
    if mask_all_condensates.sum() > 0:  # Only proceed if there are condensates
        ...
        condensateID = 1
        for cnt in contours:
            # Ignore contour if it's too large
