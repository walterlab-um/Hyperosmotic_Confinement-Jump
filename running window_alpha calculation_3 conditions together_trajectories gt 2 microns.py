    ...
    for csv_file in track(csv_files, description="Processing CSV files"):
        try:
            ...
            df_current_file = df_current_file.dropna(
                subset=["POSITION_X", "POSITION_Y", "TRACK_ID"]
            )
            df_current_file.sort_values(
                by="POSITION_T", inplace=True, ignore_index=True
            )
            ...
            valid_tracks = track_counts[
                (track_counts >= 20) & (track_counts <= 500)
            ].index
            ...
            for trackID in valid_tracks:
                ...
                x_start, x_end = (
                    df_track.iloc[0]["POSITION_X"],
                    df_track.iloc[-1]["POSITION_X"],
                )
                y_start, y_end = (
                    df_track.iloc[0]["POSITION_Y"],
                    df_track.iloc[-1]["POSITION_Y"],
                )
                total_displacement = np.sqrt(
                    (x_end - x_start) ** 2 + (y_end - y_start) ** 2
                )

                # Filter out if total displacement is less than 2 microns
...
csv_file_path = os.path.join(save_path, "alpha_nocodazole_displacement gt 2.csv")
